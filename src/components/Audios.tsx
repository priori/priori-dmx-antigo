...
export class Audios extends React.Component<AudiosProps, AudiosState> {
  ...
  newProps() {
    if (!this.oldProps) {
      if (
        this.props.player.arquivo &&
        this.els[this.props.player.arquivo] &&
      ...
      if (
        this.oldProps.player.state != this.props.player.state ||
  ...
  private pause(arquivo: string) {
    if (this.els[arquivo]) this.els[arquivo].pause();
  }

  private play(arquivo: string) {
    if (this.els[arquivo]) this.els[arquivo].play();
  }
