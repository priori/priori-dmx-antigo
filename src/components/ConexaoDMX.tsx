...
export interface ConexaoDMXProps {
  conectado: boolean;
  driver: string;
  deviceId: string;
}
export class ConexaoDMX extends React.Component<
  ConexaoDMXProps,
  { conectando: boolean }
> {
  ...
  static getDerivedStateFromProps(
    nextProps: { conectado: boolean },
    prevState: { conectando: boolean }
  ) {
  ...
  render() {
    return (
      <div className="conexao-dmx">
        <select
          defaultValue={this.props.driver}
          ref={el => (this.driverEl = el)}
        >
          {options.map(op => (
  ...
  conectar() {
    ...
    action({
      type: "dmx-conectar",
      driver: this.driverEl.value,
      deviceId: this.deviceIdEl.value
    });
  }
  desconectar() {
    if (this.props.conectado) action({ type: "dmx-desconectar" });
  }
}
