...
> {
  ...
  render() {
    ...
    for (let c = 0; c < rowsCount; c++) {
      ...
      for (let c2 = 0; c2 < colsCount; c2++) {
        row.push(
          equipamentos.filter(e => e.row == c + 1 && e.col == c2 + 1).length
        );
      }
      mapa.push(row);
      rows.push(
        equipamentos.filter(e => e.row == c + 1 && typeof e.col == "undefined")
          .length
      );
    }

    const equipamentosSelecionadosNoMapa =
      this.state.selections.length == 0
        ? this.props.equipamentos
        : this.props.equipamentos.filter(
            e =>
              this.state.selections.filter(
                s => s.col === e.col && s.row === e.row
              ).length
          );

    const equipamentosSelecionados = equipamentosSelecionadosNoMapa.filter(
      g =>
      ...
      <div>
        ...
        >
          {semPosicaoCount ? (
            <div
              className={
                this.state.selections.filter(
                  s => s.col === undefined && s.row === undefined
                ).length
                  ? "equipamentos__mapa__equipamentos-selecionados"
                  : "equipamentos__mapa__equipamentos"
              }
              style={{
                paddingTop: "10px",
                paddingBottom: "5px",
              ...
              onClick={() =>
                this.addSelection({ row: undefined, col: undefined })
              }
            >
              {semPosicaoCount
            ...
            >
              {count ? (
                <div
                  ...
                  className={
                    this.state.selections.filter(
                      s => s.col === undefined && s.row === index + 1
                    ).length
                      ? "equipamentos__mapa__equipamentos-selecionados"
                      : "equipamentos__mapa__equipamentos"
                  }
                  onClick={() =>
                    this.addSelection({ row: index + 1, col: undefined })
                  }
                >
                  {count
                    ? times(count).map((_, k) => (
              ...
              <div style={{ display: "flex" }}>
                {mapa[index].map((row, col) => (
                  <div
                    key={col}
                    onClick={() =>
                      this.addSelection({ row: index + 1, col: col + 1 })
                    }
                    className={
                      "equipamentos__mapa__celula " +
                      (this.state.selections.filter(
                        s => s.col === col + 1 && s.row === index + 1
                      ).length
                        ? "equipamentos__mapa__equipamentos-selecionados"
                        : "equipamentos__mapa__equipamentos")
                    }
        ...
        >
          <div
            className={
              "equipamento-option" +
              (this.state.selected.length == 0 ? " selected" : "")
            }
            onClick={() => this.setState({ ...this.state, selected: [] })}
            ...
            <div
              ...
              onClick={() => {
                if (this.state.selected.indexOf(g.uid) == -1)
                  this.setState({
                    ...this.state,
                    selected: [...this.state.selected, g.uid]
                  });
                else
                  this.setState({
                    ...this.state,
                    selected: this.state.selected.filter(uid => uid != g.uid)
                  });
              }}
            >
  ...
  addSelection({ col, row }: { col?: number; row?: number }) {
    if (
      this.state.selections.filter(s => s.col === col && s.row === row).length
    ) {
      this.setState({
        ...this.state,
        selections: this.state.selections.filter(
